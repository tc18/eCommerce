...
function loadCategory() {
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4) {
			...
			for (i = 0; i < jsonObj.length; i++) {
				...
				if (category.value == createOptionCategoryId) {
					createOption.setAttribute("selected", "selected");
				}

				document.offerForm.categoryId.options.add(createOption);
	...
	xmlhttp.open("get",	"${pageContext.request.contextPath}/OfferController?flag=loadingCategory&mainTypeId="+ mainTypeId.value, true);
	xmlhttp.send();
...
function removeAllCategory() {
	var removeCategory = document.offerForm.categoryId.options.length;
	for (i = removeCategory; i >= 0; i--) {
		document.offerForm.categoryId.remove(i);
	}
}
</script>
<script>
	function loadSubCategory() {
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4) {
				...
				for (i = 0; i < jsonObj.length; i++) {
					...
					document.offerForm.subCategoryId.options.add(createOption);
		...
		xmlhttp
				.open(
						"get",
						"${pageContext.request.contextPath}/OfferController?flag=loadingSubCategory&categoryId="+categoryId.value, true);
		xmlhttp.send();
		/* jQuery(".chosen-select1").chosen({'width':'100%','white-space':'nowrap'}); */
		/* Holds the status of the XMLHttpRequest. Changes from 0 to 4:
			0: request not initialized
	...
	function removeAllSubCategory() {
		var removeSubCategory = document.offerForm.subCategoryId.options.length;
		for (i = removeSubCategory; i > 0; i--) {
			document.offerForm.subCategoryId.remove(i);
		}
	}
</script>
<script>
	function loadProduct() {
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4) {
				...
				for (i = 0; i < jsonObj.length; i++) {
					var createOption = document.createElement("option");
					var createOptionProductId = jsonObj[i].productId;
					var createOptionProductName = jsonObj[i].productName;
					createOption.value = createOptionProductId;
					createOption.text = createOptionProductName;
					if (product.value == createOptionProductId) {
						createOption.setAttribute("selected", "selected");
					}

					document.offerForm.productId.options.add(createOption);
		...
		xmlhttp
				.open(
						"get",
						"${pageContext.request.contextPath}/OfferController?flag=loadingProduct&subCategoryId="
								+ subCategoryId.value, true);
		xmlhttp.send();
		/* jQuery(".chosen-select1").chosen({'width':'100%','white-space':'nowrap'}); */
		/* Holds the status of the XMLHttpRequest. Changes from 0 to 4:
			0: request not initialized
	...
	function removeAllProduct() {
		var removeProduct = document.offerForm.subCategoryId.options.length;
		for (i = removeProduct; i >=0; i--) {
			document.offerForm.productId.remove(i);
		}
	}
</script>
<script>
	...
	function validate() {
		...
		xmlhttp.open("get",
				"${pageContext.request.contextPath}/OfferController?flag=validate&offerName="
						+ offerName.value, true);
		xmlhttp.send();
	}
					...
					<div class="col-md-12">
						...
						<div class="portlet light portlet-fit portlet-form bordered">
							...
							<div class="portlet-body">
								<!-- BEGIN FORM-->
								<c:forEach items="${sessionScope.editOffer }" var="i">
									<form action="<%=request.getContextPath()%>/OfferController"
										onsubmit="return returnValidate()" name="offerForm"
										...
										<div class="form-body">
											...
											<div class="form-group">
												...
												<div class="col-md-4">
													<select class="form-control" id="mainTypeId" onChange="loadCategory()"
														name="mainTypeId" required="">
											...
											<div class="form-group">
												...
												<input type="hidden" id="category1" value="${i.categoryVO.categoryId}">
												<div class="col-md-4">
													<select class="form-control" id="categoryId" required=""
														name="categoryId" onchange="return loadSubCategory()"
														required="">
														<option>select..</option>
											...
											<div class="form-group">
												<label class="control-label col-md-3">Product <span
													class="required"> * </span>
												</label>
												<div class="col-md-4">
													<select class="form-control" id="product"
														name="productId" required="">
<%-- 														<option value="${i.productVO.productId}">${i.productVO.productName}</option> --%>
													</select>
												</div>
											</div>
											<div class="form-group">
												<label class="control-label col-md-3">Offer Name <span
													class="required"> * </span>
												</label>
												<div class="col-md-4">
													<input type="text" name="offerName" id="offerName"
														data-required="1" onblur="validate()" class="form-control"
														value="${i.offerName}" required="" />
												</div>
											...
											<div class="form-group">
												...
												<div class="col-md-4">
													<textarea class="form-control" name="offerDescription">${i.offerDescription }</textarea>
												</div>
											</div>
											<div class="form-group">
												<label class="control-label col-md-3">Date-From <span
													class="required"> * </span>
												</label>
												<div class="col-md-4">
													<input type="date" name="offerDateFrom"
														value="${i.offerDateFrom }" data-required="1"
														class="form-control" required="" />
												</div>
											</div>
											<div class="form-group">
												<label class="control-label col-md-3">Date-To <span
													class="required"> * </span>
												</label>
												<div class="col-md-4">
													<input type="date" name="offerDateTo" data-required="1"
														value="${i.offerDateTo }" class="form-control" required="" />
												</div>
											</div>
										</div>
											...
											<div class="row">
												<div class="col-md-offset-3 col-md-9">
													<input type="hidden" id="subCategory"
														value="${i.subCategoryVO.subCategoryId }" /> <input type="hidden"
														id="product" value="${i.productVO.productId }" />
